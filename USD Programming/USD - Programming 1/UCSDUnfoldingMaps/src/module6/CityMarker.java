...
public class CityMarker extends CommonMarker {
	...
	public void showTitle(PGraphics pg, float x, float y)
	{
		...
		String dmg = "Est dmg: " + getDamage() + " Million";
		...
		pg.rect(x, y-TRI_SIZE-39, Math.max(pg.textWidth(name), pg.textWidth(dmg)) + 6, 54);
		pg.fill(0, 0, 0);
		pg.textAlign(PConstants.LEFT, PConstants.TOP);
		pg.text(name, x+3, y-TRI_SIZE-33);
		pg.text(pop, x+3, y - TRI_SIZE -18);
		pg.text(dmg, x+3, y - TRI_SIZE -3);
	...
	float getDamage()
	{
		return Float.parseFloat(getStringProperty("damage"));
	}
}
