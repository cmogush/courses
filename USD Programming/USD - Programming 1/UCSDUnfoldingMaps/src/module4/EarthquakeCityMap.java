...
public class EarthquakeCityMap extends PApplet {
	...
	private boolean isLand(PointFeature earthquake) {		
		...
		for (Marker m : countryMarkers) {
			// TODO-DONE: Finish this method using the helper method isInCountry
	...
	private void printQuakes() 
	{
		...
		System.out.println("countryName : noQuakes");
		
		for(Marker cm : countryMarkers) {
			String countryName = (String)cm.getProperty("name");
			int quakeCounter = 0;
			for(Marker em : quakeMarkers) {
				if(em instanceof LandQuakeMarker) { //if em is a landQuakeMarker, then compare it against cm
		...
		for(Marker em : quakeMarkers) {
			if(em instanceof OceanQuakeMarker) { //if em is a landQuakeMarker, then compare it against cm
				oceanQuakeCounter++;
			}
		}	
