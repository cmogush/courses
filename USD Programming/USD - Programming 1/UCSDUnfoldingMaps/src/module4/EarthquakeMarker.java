...
public abstract class EarthquakeMarker extends SimplePointMarker
{
	...
	public EarthquakeMarker (PointFeature feature) 
	{
		...
		if(magnitude < THRESHOLD_LIGHT) { //minor earthquake
			properties.put("radius", 2*magnitude );
			setProperties(properties);
			this.radius = 1.0f*getMagnitude();
	    }
	    if(magnitude >= THRESHOLD_LIGHT && magnitude < THRESHOLD_MODERATE) { //light quake
	    	properties.put("radius", 2*magnitude );
			setProperties(properties);
			this.radius = 1.25f*getMagnitude();
	    }
	    if(magnitude >= THRESHOLD_MODERATE) { //moderate quake
	    	properties.put("radius", 2*magnitude );
			setProperties(properties);
			this.radius = 1.5f*getMagnitude();
	    }
	    if(properties.get("age").equals("Past Hour") || properties.get("age").equals("Past Day")) {
	    	recent = true;
	    }
	}
	...
	private void colorDetermine(PGraphics pg) {
		...
		if(depth > 70 && depth < 300) {; //intermediate
